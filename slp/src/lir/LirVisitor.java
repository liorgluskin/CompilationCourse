...
public class LirVisitor implements PropagatingVisitor<Environment,LirReturnInfo>{
	...
	private void lirAssignHandler(String location, String value, int lineNum, Environment d){
		...
		if(location.startsWith("R") && /*added by lior */!location.contains(".")){
	...
	public LirReturnInfo visit(VarLocation var_loc, Environment d) {
		...
		if (var_loc.getLocation() != null){
			...
			if(location_expr.getRegisterLocation().charAt(0)!= 'R'){
				d.addLirInstruction(location_expr.getMoveCommand().toString(), location_expr.getRegisterLocation(),loc);
				d.incrementRegister();
			}else{
			...
			//runtime check
			d.addLirInstruction("StaticCall", "__checkNullRef(a="+loc+")", "Rdummy");
	...
	public LirReturnInfo visit(ArrLocation arr_loc, Environment d) {
		...
		if(index.getRegisterLocation().charAt(0) != 'R'){
			...
			d.incrementRegister();
			d.addLirInstruction(index.getMoveCommand().toString(), index.getRegisterLocation(),indexLoc);		
		} else{
			indexLoc = index.getRegisterLocation();
		}

		//runtime check
		d.addLirInstruction("StaticCall", "__checkArrayAccess(a="+arrayLoc+",i="+indexLoc+")","Rdummy");
	...
	public LirReturnInfo visit(StaticCall static_call, Environment d) {
		...
		for (Expr arg: static_call.getArguments()){
			...
			//check if arg_expr ia a field
			if(arg_expr.getMoveCommand() == MoveEnum.MOVE_FIELD){
				//transfer it to a new register
				d.addLirInstruction(MoveEnum.MOVE_FIELD, arg_expr.getRegisterLocation(), argLoc);
				d.incrementRegister();
			}else{
		...
		//library method call
		if (static_call.getClassName().equals("Library")){
			d.addLirInstruction("Library", code, resReg);
		}
		else{
			d.addLirInstruction("StaticCall", code, resReg);
		}
	...
	public LirReturnInfo visit(VirtualCall virtual_call, Environment d) {
		...
		if (obj_ref != null){
			...
			if(location.getRegisterLocation().charAt(0)!= 'R' || location.getRegisterLocation().contains("[")){
				...
				d.incrementRegister();
				d.addLirInstruction(location.getMoveCommand().toString(),location.getRegisterLocation(),objLoc);
	...
	public LirReturnInfo visit(NewObject new_obj, Environment d) {
		...
		d.addLirInstruction("Library","__allocateObject("+size + ")",reg);
		d.addLirInstruction(MoveEnum.MOVE_FIELD,"_DV_" + class_name,reg+".0");
	...
	public LirReturnInfo visit(NewArray new_arr, Environment d) {
		...
		d.addLirInstruction("Mul","4",reg);

		//runtime check
		d.addLirInstruction("StaticCall","__checkSize(n="+reg+")","Rdummy");

		//library function - allocate memory for a new array
		d.addLirInstruction("Library","__allocateArray("+reg+")",reg);
	...
	public LirReturnInfo visit(BinaryOpExpr expr, Environment d) {
		...
		if (lhs_type.toString().compareTo("int") != 0){
			d.addLirInstruction("Library","__stringCat("+operand1.getRegisterLocation()+","+operand2.getRegisterLocation()
		...
		d.incrementRegister();
		d.addLirInstruction(operand1.getMoveCommand(), operand1.getRegisterLocation(), resOp);
